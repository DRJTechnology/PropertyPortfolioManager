...
AS
	...
	/* Tenancy Types - start */
	IF NOT EXISTS (SELECT 1 FROM property.[TenancyType] WHERE PortfolioId = @PortfolioId AND [Type] = 'Assured Shorthold Tenancy (AST)')
	BEGIN
		INSERT INTO property.[TenancyType] (PortfolioId, [Type], Active, Deleted, [CreateUserId], [CreateDate], [AmendUserId], [AmendDate])
		VALUES (@PortfolioId, 'Assured Shorthold Tenancy (AST)', 1, 0, 1, SYSDATETIME(), 1, SYSDATETIME())
	END
	IF NOT EXISTS (SELECT 1 FROM property.[TenancyType] WHERE PortfolioId = @PortfolioId AND [Type] = 'Company Let Agreement')
	BEGIN
		INSERT INTO property.[TenancyType] (PortfolioId, [Type], Active, Deleted, [CreateUserId], [CreateDate], [AmendUserId], [AmendDate])
		VALUES (@PortfolioId, 'Company Let Agreement', 1, 0, 1, SYSDATETIME(), 1, SYSDATETIME())
	END
	/* Tenancy Types - end */
